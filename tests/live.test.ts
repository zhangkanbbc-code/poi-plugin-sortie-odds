...
describe('deriveLiveSortie', () => {
  ...
  it('战斗中状态纯从 poi 核心 battle._status.battle 派生', () => {
    const sortie = {
      sortieMapId: '15',
      sortieStatus: [true, false, false, false],
      spotHistory: [0, 3, 7],
    ...
    const idle = deriveLiveSortie(initialState, sortie, { _status: { battle: null } })
    expect(idle.battleOngoing).toBe(false)
    // battle 参数缺失时不再退回自有记账（那套已被移除）——直接视为不在战斗中
    expect(deriveLiveSortie(initialState, sortie).battleOngoing).toBe(false)
  })
